...
@Component({
  selector: 'app-aventuramatematica',
  templateUrl: './aventuramatematica.component.html',
  styleUrls: ['./aventuramatematica.component.scss']
})
export class AventuramatematicaComponent implements OnInit,OnDestroy {
  usuario:any = null
  ...
  ReiniciarJuego()
  {
    ...
    this.opciones = this.GenerarOpciones().sort(() => Math.random() - 0.5);
    this.IniciarContador()
    this.notifyService.showInfo('Juego Reiniciado', 'Aventura Matematica');
  }
  ...
  Resolver(respuesta:number,event:Event) {
    if(this.juegoActivo)
    {
      ...
      if (respuesta === this.EvaluarOperacion()) {
        ...
        this.notifyService.showSuccess('¡Correcto!', 'Aventura Matematica');
      } else {
